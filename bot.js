...
async function getUserTakeCount(userId) {
  const { count, error } = await supabase
    .from('daily_takes')
    .select('*', { count: 'exact', head: true })
    .eq('user_id', userId);
  if (error) { console.error('getUserTakeCount error:', error); return 0; }
  return count;
}
...
client.on('interactionCreate', async interaction => {
  ...
  const userTakeCount = await getUserTakeCount(member.id);
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Take Counter', value: `**#${userTakeCount.toLocaleString()}**`, inline: true },
